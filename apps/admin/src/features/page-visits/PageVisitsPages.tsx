...
const entity='page-visits',config=entityMap[entity];
export function PageVisitsList(){return <EntityListView entity={entity} config={config}/>}
export function PageVisitsCreate(){return <EntityCreateView entity={entity} config={config}/>}
export function PageVisitsDetail(){const {id=''}=useParams();const {data,isLoading,error}=useGetOneQuery({entity,id});if(isLoading)return <div className="empty-state">Loading record…</div>;if(error||!data)return <div className="empty-state">Record not found.</div>;
