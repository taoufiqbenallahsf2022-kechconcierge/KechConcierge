...
const entity='accounts',config=entityMap[entity];
export function AccountsList(){return <EntityListView entity={entity} config={config}/>}
export function AccountsCreate(){return <EntityCreateView entity={entity} config={config}/>}
export function AccountsDetail(){const {id=''}=useParams();const {data,isLoading,error}=useGetOneQuery({entity,id});if(isLoading)return <div className="empty-state">Loading record…</div>;if(error||!data)return <div className="empty-state">Record not found.</div>;
