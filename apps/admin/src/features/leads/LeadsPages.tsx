...
const entity='leads',config=entityMap[entity];
export function LeadsList(){return <EntityListView entity={entity} config={config}/>}
export function LeadsCreate(){return <EntityCreateView entity={entity} config={config}/>}
export function LeadsDetail(){const {id=''}=useParams();const {data,isLoading,error}=useGetOneQuery({entity,id});if(isLoading)return <div className="empty-state">Loading record…</div>;if(error||!data)return <div className="empty-state">Record not found.</div>;
