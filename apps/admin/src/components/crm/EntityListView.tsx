...
function FilterInput(p:{field?:Field;textValue:string;setTextValue:(v:string)=>void;booleanValue:string;setBooleanValue:(v:string)=>void;dateOperator:DateOperator;setDateOperator:(v:DateOperator)=>void;dateFrom:string;setDateFrom:(v:string)=>void;dateTo:string;setDateTo:(v:string)=>void}){
 ...
 if(p.field.kind==='enum')return <select className="filter-value" value={p.textValue} onChange={e=>p.setTextValue(e.target.value)}><option value="">Select value</option>{p.field.options?.map(v=><option key={typeof v==='string'?v:v.value} value={typeof v==='string'?v:v.value}>{typeof v==='string'?v.replaceAll('_',' '):v.label}</option>)}<option value="null">Null</option></select>;
